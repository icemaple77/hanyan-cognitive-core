...
class GenerationStats:
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            "orphans_archived": self.orphans_archived,
        }
...
class QMDGenerator:
    ...
    def _render_master_index(
        ...
        lines = [
            "# Knowledge Base",
            "",
            "> 本目录由 HCC QMDGenerator 自动生成，请勿手编；双向同步由 HCC SyncEngine 管理。",
            "",
    ...
    # ------------------------------------------------------------------
    # Orphan archiving (memories deleted/unshared since the last run)
    # ------------------------------------------------------------------
    def _archive_orphans(self, orphans: set[Path]) -> int:
        """Move ``orphans`` (stale content files) under ``HCC_ARCHIVE_DIR``.

        The archive dir is excluded from ``scripts/index_documents.py`` on
        purpose (see its ``EXCLUDE_DIRS``), so archiving here — instead of
        deleting or leaving the stale file in place under ``Knowledge/`` — is
        what actually keeps the indexer's view of the knowledge base in sync
        with PostgreSQL without losing the content.
        """
        if not orphans:
            return 0
        archive_root = Path(self._settings.archive_dir).expanduser() / "Knowledge"
        moved = 0
        for path in orphans:
            try:
            ...
            try:
                ...
                logger.exception("failed to archive orphan QMD file %s", path)
        if moved:
            logger.info("archived %s orphan QMD file(s) to %s", moved, archive_root)
        return moved
    ...
    async def generate_all(self) -> GenerationStats:
        ...
        # Snapshot pre-existing content files (excl. READMEs) so any that
        # disappear from this run's write-set can be archived rather than
        # left stale (memory deleted/unshared since the last generation).
        existing_before = {
            p for p in self.root.rglob("*.md") if p.is_file() and p.name.lower() != "readme.md"
        ...
        stats.orphans_archived = self._archive_orphans(existing_before - written_paths)
